# EKS Info WebApp - EKS 环境信息展示应用

这是一个部署在 Amazon EKS 上的 Python Flask Web 应用，用于演示和展示 EKS 环境信息、AWS 存储服务访问（EBS、EFS、S3）以及 Kubernetes 自动扩展能力。

## 作者信息

- **作者**: RJ.Wang
- **邮箱**: wangrenjun@gmail.com
- **创建时间**: 2025-11-13
- **最后更新**: 2025-11-16
- **项目**: EKS Info WebApp - EKS 环境信息展示应用
- **版本**: v1.0-cleanup-complete

## 项目状态

✅ **项目清理完成** (2025-11-16)
- 所有冗余文件已清理
- 文档结构已优化
- 链接验证已完成
- 代码质量已提升
- 项目已标记版本 v1.0-cleanup-complete

## 应用功能

### 核心功能
- 🖥️ **环境信息展示**: Pod、Node、集群、EC2 实例、网络架构信息
- 💾 **存储服务演示**: EBS、EFS、S3 三种存储服务的实际访问演示
- 📊 **资源监控**: Kubernetes 资源使用情况和状态监控
- 🔄 **自动扩展**: HPA 和 Cluster Autoscaler 演示
- 🧪 **压力测试**: CPU 和内存压力测试，触发自动扩展
- 🌐 **网络可视化**: VPC、子网、安全组、路由表信息展示

### 存储功能
- **EBS (块存储)**: 单 Pod 持久化日志，演示 ReadWriteOnce 访问模式
- **EFS (共享文件系统)**: 多 Pod 共享文件，演示 ReadWriteMany 访问模式
- **S3 (对象存储)**: 通过 IRSA 安全访问，演示对象上传、下载、删除

### 技术特性
- 🚀 **一键部署**: 自动创建 EKS 集群、VPC、应用等所有资源
- 🧹 **一键清理**: 自动删除所有创建的 AWS 资源，避免产生不必要费用
- 📊 **智能恢复**: 自动检测和恢复部署问题
- 💰 **成本透明**: 清晰显示预估成本和资源使用情况
- 🔧 **工具检查**: 自动检查所需工具和 AWS 凭证
- 📋 **详细日志**: 提供详细的部署信息和管理命令
- 🏗️ **ARM64 支持**: 运行在 AWS Graviton 处理器上，性能和成本优化

## 应用架构

### 整体架构
```
┌─────────────────────────────────────────────────────────────┐
│                      Internet / User                        │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                  Application Load Balancer                  │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                    Kubernetes Ingress                       │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                  Kubernetes Service (ClusterIP)             │
└──────────────────────────┬──────────────────────────────────┘
                           │
        ┌──────────────────┼──────────────────┐
        ▼                  ▼                  ▼
    ┌───────┐          ┌───────┐          ┌───────┐
    │ Pod 1 │          │ Pod 2 │          │ Pod 3 │
    │       │          │       │          │       │
    │ Flask │          │ Flask │          │ Flask │
    │  App  │          │  App  │          │  App  │
    └───┬───┘          └───┬───┘          └───┬───┘
        │                  │                  │
        └──────────────────┼──────────────────┘
                           │
        ┌──────────────────┼──────────────────┐
        │                  │                  │
        ▼                  ▼                  ▼
    ┌───────┐          ┌───────┐          ┌───────┐
    │  EBS  │          │  EFS  │          │  S3   │
    │ (PVC) │          │(Shared)│         │(IRSA) │
    └───────┘          └───────┘          └───────┘
```

### 网络架构
```
┌─────────────────────────────────────────────────────────────┐
│                        AWS VPC                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌──────────────┐ │
│  │   Public AZ-A   │  │   Public AZ-B   │  │ Public AZ-C  │ │
│  │                 │  │                 │  │              │ │
│  │   NAT Gateway   │  │   NAT Gateway   │  │ NAT Gateway  │ │
│  └─────────────────┘  └─────────────────┘  └──────────────┘ │
│  ┌─────────────────┐  ┌─────────────────┐  ┌──────────────┐ │
│  │  Private AZ-A   │  │  Private AZ-B   │  │ Private AZ-C │ │
│  │                 │  │                 │  │              │ │
│  │  EKS Nodes      │  │  EKS Nodes      │  │  EKS Nodes   │ │
│  │  + EFS Mount    │  │  + EFS Mount    │  │  + EFS Mount │ │
│  └─────────────────┘  └─────────────────┘  └──────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

## 部署的资源

### 网络基础设施
- **VPC**: 10.101.0.0/16 CIDR
- **公有子网**: 3个可用区，每个 /24
- **私有子网**: 6个子网（每个可用区2个），每个 /24
- **NAT Gateway**: 3个（每个可用区1个）
- **Internet Gateway**: 1个
- **路由表**: 公有和私有路由表

### EKS 集群
- **EKS 集群**: Kubernetes 1.31
- **节点组**: t3.medium ARM64 实例，2-4个节点
- **IAM 角色**: 集群和节点组所需的角色和策略
- **安全组**: 集群、节点、EFS 的安全组配置
- **OIDC Provider**: 用于 IRSA 认证

### 存储资源
- **EFS 文件系统**: 加密的共享文件系统，带挂载目标
- **S3 存储桶**: 加密的对象存储，配置生命周期策略
- **EBS CSI Driver**: 通过 Helm 安装，支持动态卷供应
- **EFS CSI Driver**: 通过 Helm 安装，支持共享文件系统
- **StorageClass**: EBS gp3 和 EFS 存储类
- **PVC**: EBS (10Gi) 和 EFS (20Gi) 持久卷声明

### 应用组件
- **Namespace**: rj-webdemo
- **ServiceAccount**: 配置 IRSA 注解，用于 S3 访问
- **Deployment**: EKS Info App，3个副本，配置资源限制和健康检查
- **Service**: ClusterIP 服务
- **Ingress**: ALB Ingress Controller 配置
- **HPA**: 水平 Pod 自动扩展器（3-10 副本）
- **Load Balancer**: Application Load Balancer

### IAM 和权限
- **EKS Info App IAM 角色**: S3 访问和 AWS 服务描述权限
- **CSI Driver IAM 角色**: EBS 和 EFS 操作权限
- **ALB Controller IAM 角色**: 负载均衡器管理权限
- **IRSA 配置**: 安全的临时凭证机制

## 前置要求

### 必需工具
- [Terraform](https://www.terraform.io/downloads.html) >= 1.3.2
- [kubectl](https://kubernetes.io/docs/tasks/tools/) >= 1.28
- [Helm](https://helm.sh/docs/intro/install/) >= 3.0
- [AWS CLI](https://aws.amazon.com/cli/) >= 2.0

### AWS 配置
```bash
# 配置 AWS 凭证
aws configure

# 或设置环境变量
export AWS_ACCESS_KEY_ID="your-access-key"
export AWS_SECRET_ACCESS_KEY="your-secret-key"
export AWS_DEFAULT_REGION="ap-southeast-1"
```

### IAM 权限
确保 AWS 用户具有以下权限：
- EC2 完全访问权限
- EKS 完全访问权限
- IAM 角色和策略管理权限
- VPC 和网络资源管理权限
- Application Load Balancer 管理权限

## 快速开始

### 步骤 1: 构建和推送 Docker 镜像

```bash
# 构建 ARM64 镜像并推送到 ECR
./scripts/build.sh

# 或者指定标签
./scripts/build.sh -t v1.0

# 查看帮助
./scripts/build.sh --help
```

### 步骤 2: 部署集群和应用

```bash
# 一键部署（推荐）
./scripts/deploy.sh

# 或明确指定部署
./scripts/deploy.sh deploy
```

部署过程包括：
1. ✅ 工具和凭证检查
2. 🔧 Terraform 初始化
3. 📋 生成执行计划
4. ⚠️ 确认部署（需要输入 'yes'）
5. 🚀 创建基础设施（15-20分钟）
   - VPC 和网络资源
   - EKS 集群和节点组
   - EFS 文件系统和 S3 存储桶
   - CSI Driver 安装
6. 🔧 配置 kubectl
7. 📦 部署应用和 Kubernetes 资源
8. ⏳ 等待应用就绪
9. 📊 显示访问信息

## 部署方式说明

### 标准部署（推荐）✅

本项目使用 **Terraform** 管理所有 AWS 和 Kubernetes 资源，这是推荐的部署方式：

```bash
./scripts/deploy.sh
```

**Terraform 管理的资源包括**：
- ✅ VPC 和网络资源（子网、路由表、NAT Gateway 等）
- ✅ EKS 集群和节点组
- ✅ IAM 角色和策略
- ✅ EFS 文件系统和 S3 存储桶
- ✅ **ECR 仓库**（由 Terraform 自动创建）
- ✅ Kubernetes 资源（Deployment、Service、Ingress 等）
- ✅ Helm Charts（CSI Drivers、ALB Controller）

### 智能恢复机制

部署脚本包含智能恢复机制，当 Terraform Kubernetes Provider 遇到问题时，会自动切换到备用模式：

1. **AWS 基础设施**：由 Terraform 创建 ✅
2. **Kubernetes 资源**：通过 kubectl 和 Helm 创建 ⚠️
3. **资源状态**：不在 Terraform 状态管理中 ⚠️

**注意**：这是自动恢复机制，不需要手动干预。如果触发了备用模式，建议修复 Terraform 配置后重新部署。

### k8s/ 目录说明

`k8s/` 目录包含独立的 Kubernetes 清单文件，用于：
- 🔄 备用部署方案（自动触发）
- 🧪 开发和测试
- 🔍 故障排查

**⚠️ 重要**：不要直接使用 `kubectl apply -f k8s/` 进行生产部署，这会绕过 Terraform 管理。

### 步骤 3: 访问应用

部署完成后，访问输出的 ALB 地址：

```
🌐 应用访问地址:
   http://your-alb-hostname.region.elb.amazonaws.com
```

应用页面包括：
- **首页 (/)**: 环境信息展示
- **/storage**: 存储概览
- **/ebs**: EBS 块存储演示
- **/efs**: EFS 共享文件系统演示
- **/s3**: S3 对象存储演示
- **/network**: 网络架构信息
- **/resources**: Kubernetes 资源信息
- **/stress**: 压力测试和自动扩展演示
- **/scaling**: 扩展监控和历史

### 步骤 4: 清理所有资源

```bash
./scripts/deploy.sh clean
```

清理过程包括：
1. ⚠️ 确认删除（需要输入 'yes'）
2. 🗑️ 删除 Kubernetes 资源
3. 🗑️ 删除所有 AWS 资源（10-15分钟）
4. 🧹 清理本地文件

### 查看帮助

```bash
# 部署脚本帮助
./scripts/deploy.sh help

# 构建脚本帮助
./scripts/build.sh --help
```

## 成本估算

### 主要费用组件
- **EKS 集群**: ~$0.10/小时
- **EC2 实例**: t3.medium × 2-4个 ≈ $0.08-0.16/小时
- **NAT Gateway**: 3个 × $0.045/小时 ≈ $0.135/小时
- **Application Load Balancer**: ~$0.025/小时
- **数据传输**: 根据使用量

**总计**: 约 $2-4/小时（根据区域和使用情况）

⚠️ **重要**: 测试完成后请及时运行 `./scripts/deploy.sh clean` 清理资源！

## 部署后管理

### 访问应用
部署完成后，脚本会显示应用的访问地址：
```
🌐 应用访问地址:
   http://your-alb-hostname.region.elb.amazonaws.com
```

### 常用 kubectl 命令
```bash
# 查看 Pod 状态
kubectl get pods -n rj-webdemo

# 查看服务
kubectl get services -n rj-webdemo

# 查看 Ingress
kubectl get ingress -n rj-webdemo

# 扩缩容应用
kubectl scale deployment rj-py-webdemo --replicas=5 -n rj-webdemo

# 查看 Pod 日志
kubectl logs -f deployment/rj-py-webdemo -n rj-webdemo
```

### Terraform 命令
```bash
# 进入 Terraform 目录
cd terraform

# 查看输出信息
terraform output

# 查看状态
terraform state list

# 查看特定资源
terraform show

# 返回项目根目录
cd ..
```

## 故障排除

### 常见问题

#### 1. Docker 镜像构建失败

**问题**: Docker 镜像源访问失败
```
ERROR: failed to resolve reference "docker.io/library/python:3.11-slim"
```

**解决方案**:
- 临时禁用 Docker 镜像加速器
- 编辑 `~/.docker/daemon.json`，注释掉 `registry-mirrors`
- 重启 Docker Desktop 或 Docker 服务
- 详见 `eks-info-app/DOCKER_BUILD_GUIDE.md`

#### 2. Terraform 部署失败

**问题**: Kubernetes provider 认证错误
```
Error: getting credentials: decoding stdout: couldn't get version/kind
```

**解决方案**:
- 脚本会自动检测并切换到混合部署模式
- 基础设施由 Terraform 管理
- Kubernetes 资源通过 kubectl/Helm 部署
- 无需手动干预，脚本会自动恢复

#### 3. 应用无法访问

**问题**: ALB 地址无法访问

**解决方案**:
- 等待 ALB 完全就绪（2-3分钟）
- 检查 Pod 状态：`kubectl get pods -n rj-webdemo`
- 检查 Ingress 状态：`kubectl describe ingress -n rj-webdemo`
- 查看 ALB Controller 日志：
  ```bash
  kubectl logs -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller
  ```

#### 4. 存储挂载失败

**问题**: Pod 无法挂载 EBS 或 EFS

**解决方案**:
- 检查 CSI Driver 状态：
  ```bash
  kubectl get pods -n kube-system | grep csi
  ```
- 检查 PVC 状态：
  ```bash
  kubectl describe pvc -n rj-webdemo
  ```
- 验证 IAM 角色权限
- 检查安全组配置（EFS 需要 NFS 端口 2049）

#### 5. IRSA 权限问题

**问题**: S3 访问被拒绝

**解决方案**:
- 验证 ServiceAccount 注解：
  ```bash
  kubectl get sa eks-info-app-sa -n rj-webdemo -o yaml
  ```
- 检查 Pod 环境变量：
  ```bash
  kubectl exec -n rj-webdemo -it <pod-name> -- env | grep AWS
  ```
- 验证 IAM 角色信任策略
- 确认 OIDC Provider 配置正确

#### 6. kubectl 连接失败

**问题**: 无法连接到 EKS 集群

**解决方案**:
- 重新配置 kubeconfig：
  ```bash
  aws --profile susermt eks update-kubeconfig \
    --region ap-southeast-1 \
    --name RJtest-eks-cluster-20250822
  ```
- 验证 AWS 凭证：
  ```bash
  aws --profile susermt sts get-caller-identity
  ```

#### 7. 清理失败

**问题**: Terraform destroy 失败

**解决方案**:
- 手动检查 AWS 控制台中的资源
- 确认没有其他依赖资源阻止删除
- 手动删除 ALB（如果存在）
- 重新运行清理命令：`./scripts/deploy.sh clean`
- 查看详细日志：`deployment.log`

### 日志查看

```bash
# 部署日志
cat deployment.log

# Terraform 详细日志
export TF_LOG=DEBUG
cd terraform
terraform apply
cd ..

# kubectl 事件
kubectl get events -n rj-webdemo

# Pod 日志
kubectl logs -n rj-webdemo -l app=eks-info-app --tail=100

# ALB Controller 日志
kubectl logs -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller --tail=100
```

### 性能调优

#### HPA 触发测试
访问 `/stress` 页面，执行 CPU 或内存压力测试：
- CPU 压力测试：持续 60 秒，强度可调
- 内存压力测试：分配指定大小内存
- 观察 HPA 自动扩展 Pod 数量

#### 手动扩展
```bash
# 手动扩展 Pod
kubectl scale deployment/eks-info-app --replicas=5 -n rj-webdemo

# 查看 HPA 状态
kubectl get hpa -n rj-webdemo

# 查看扩展事件
kubectl describe hpa eks-info-app-hpa -n rj-webdemo
```

## 应用功能详解

### 环境信息展示 (/)
- Pod 基本信息（名称、命名空间、IP、节点）
- Kubernetes 集群信息（版本、API 端点）
- EC2 实例信息（实例 ID、类型、可用区）
- 系统架构信息（ARM64 检测）
- 网络配置（VPC、子网、安全组）
- 环境变量列表

### 存储概览 (/storage)
- 所有挂载点信息
- EBS 卷配置和使用情况
- EFS 文件系统配置
- S3 存储桶配置和 IRSA 角色
- 导航到各存储演示页面

### EBS 演示 (/ebs)
- 显示 EBS 卷信息和磁盘使用率
- 写入访问日志（时间戳、内容）
- 读取历史日志记录
- 演示单 Pod 持久化存储
- 验证 Pod 重启后数据持久性

### EFS 演示 (/efs)
- 显示 EFS 文件系统信息
- 写入文件（包含 Pod 名称）
- 读取所有 Pod 写入的文件
- 演示多 Pod 共享文件系统
- 显示当前处理请求的 Pod

### S3 演示 (/s3)
- 显示 S3 存储桶信息
- 上传对象到 S3
- 列出所有对象
- 下载对象内容
- 删除对象
- 显示 IRSA 角色信息

### 网络信息 (/network)
- VPC 详细信息（CIDR、DNS 设置）
- 所有子网信息（公有/私有）
- 安全组规则（入站/出站）
- 路由表配置
- Load Balancer 信息

### Kubernetes 资源 (/resources)
- Pod 列表和状态
- Service 信息
- Deployment 配置
- 节点资源使用情况
- PV 和 PVC 状态

### 压力测试 (/stress)
- CPU 压力测试（可配置持续时间和强度）
- 内存压力测试（可配置目标内存大小）
- 实时显示 CPU 和内存使用率
- 触发 HPA 自动扩展
- 显示测试状态和结果

### 扩展监控 (/scaling)
- 当前节点列表和资源使用
- 每个节点上的 Pod 分布
- Pending 状态 Pod 检测
- HPA 当前状态和副本数
- 扩展事件历史
- 资源使用率趋势

## 技术栈

### 后端
- **Python 3.11**: 应用运行时
- **Flask 3.0**: Web 框架
- **boto3**: AWS SDK for Python
- **kubernetes**: Kubernetes Python Client
- **psutil**: 系统信息收集
- **gunicorn**: WSGI HTTP 服务器

### 基础设施
- **Terraform**: 基础设施即代码
- **AWS EKS**: Kubernetes 托管服务
- **AWS VPC**: 网络隔离
- **AWS EFS**: 共享文件系统
- **AWS S3**: 对象存储
- **AWS EBS**: 块存储

### Kubernetes 组件
- **Deployment**: 应用部署
- **Service**: 服务发现
- **Ingress**: 流量路由
- **HPA**: 自动扩展
- **PVC**: 持久卷声明
- **ServiceAccount**: IRSA 认证

### 存储驱动
- **EBS CSI Driver**: 动态 EBS 卷供应
- **EFS CSI Driver**: EFS 文件系统挂载
- **AWS Load Balancer Controller**: ALB 管理

## 文件结构

```
.
├── README.md                     # 本文档
├── DEPLOYMENT.md                 # 部署指南（合并）
├── PROJECT_SUMMARY.md            # 项目总结（合并）
├── BUGFIX_REPORT.md              # Bug 修复报告（合并）
├── TROUBLESHOOTING.md            # 故障排除
├── .gitignore                    # Git 忽略文件
│
├── terraform/                    # Terraform 代码
│   ├── main.tf                   # 基础设施定义
│   ├── app.tf                    # 应用资源定义
│   ├── variables.tf              # 变量定义
│   ├── outputs.tf                # 输出定义
│   ├── versions.tf               # Terraform 版本约束
│   ├── terraform.tfvars          # 变量值
│   ├── .terraform.lock.hcl       # Provider 版本锁定
│   └── .terraform/               # Terraform 工作目录
│
├── scripts/                      # 工具脚本
│   ├── build.sh                  # 镜像构建和推送脚本
│   ├── deploy.sh                 # 主部署脚本（智能版）
│   ├── get-alb-hostname.sh       # 获取 ALB 地址脚本
│   ├── test_all_pages.sh         # 页面测试脚本
│   ├── verify_environment_info.sh # 环境验证脚本
│   └── verify_frontend.sh        # 前端验证脚本
│
├── k8s/                          # Kubernetes 清单文件
│   ├── namespace.yaml
│   ├── serviceaccount.yaml
│   ├── deployment.yaml
│   ├── service.yaml
│   ├── ingress.yaml
│   ├── hpa.yaml
│   ├── README.md
│   └── storage/                  # 存储配置
│       ├── storageclass-ebs.yaml
│       ├── storageclass-efs.yaml
│       ├── pvc-ebs.yaml
│       └── pvc-efs.yaml
│
├── eks-info-app/                 # 应用源代码
│   ├── app.py                    # Flask 应用主文件
│   ├── config.py                 # 配置文件
│   ├── main.py                   # 应用入口
│   ├── requirements.txt          # Python 依赖
│   ├── Dockerfile                # Docker 镜像定义
│   ├── build.sh                  # Docker 构建脚本
│   ├── push-to-ecr.sh            # ECR 推送脚本
│   ├── README.md                 # 应用说明
│   ├── API_DOCUMENTATION.md      # API 文档（合并）
│   ├── DOCKER_BUILD_GUIDE.md     # Docker 构建指南
│   ├── services/                 # 服务层
│   │   ├── environment_service.py
│   │   ├── kubernetes_service.py
│   │   ├── aws_service.py
│   │   ├── storage_service.py
│   │   ├── stress_test_service.py
│   │   └── metrics_service.py
│   ├── storage/                  # 存储访问层
│   │   ├── ebs_storage.py
│   │   ├── efs_storage.py
│   │   └── s3_storage.py
│   ├── routes/                   # 路由处理
│   │   ├── home_routes.py
│   │   ├── storage_routes.py
│   │   ├── ebs_routes.py
│   │   ├── efs_routes.py
│   │   ├── s3_routes.py
│   │   ├── network_routes.py
│   │   ├── resources_routes.py
│   │   ├── stress_routes.py
│   │   └── scaling_routes.py
│   ├── templates/                # HTML 模板
│   │   ├── base.html
│   │   ├── index.html
│   │   ├── storage.html
│   │   ├── ebs.html
│   │   ├── efs.html
│   │   ├── s3.html
│   │   ├── network.html
│   │   ├── resources.html
│   │   ├── stress.html
│   │   └── scaling.html
│   ├── static/                   # 静态资源
│   │   ├── css/
│   │   └── js/
│   └── tests/                    # 测试文件
│       ├── __init__.py
│       ├── conftest.py
│       ├── test_*.py
│       └── ...
│
└── .kiro/                        # Kiro 规范文件
    └── specs/
        ├── eks-info-webapp/
        │   ├── requirements.md
        │   ├── design.md
        │   └── tasks.md
        └── project-cleanup/
            ├── requirements.md
            ├── design.md
            └── tasks.md
```

## 部署流程图

```
┌─────────────────────────────────────────────────────────────┐
│                    1. 构建 Docker 镜像                      │
│  ./scripts/build.sh                                         │
│  • 构建 ARM64 镜像                                          │
│  • 推送到 ECR                                               │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    2. 初始化 Terraform                      │
│  terraform init                                             │
│  • 下载 Provider 插件                                       │
│  • 初始化后端                                               │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    3. 创建基础设施                          │
│  terraform apply                                            │
│  • VPC 和网络 (5 分钟)                                      │
│  • EKS 集群 (10 分钟)                                       │
│  • 节点组 (5 分钟)                                          │
│  • EFS 和 S3 (2 分钟)                                       │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    4. 安装 CSI Driver                       │
│  Helm install                                               │
│  • EBS CSI Driver                                           │
│  • EFS CSI Driver                                           │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    5. 部署应用                              │
│  kubectl apply                                              │
│  • Namespace                                                │
│  • ServiceAccount (IRSA)                                    │
│  • StorageClass 和 PVC                                      │
│  • Deployment                                               │
│  • Service 和 Ingress                                       │
│  • HPA                                                      │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    6. 安装 ALB Controller                   │
│  Helm install                                               │
│  • AWS Load Balancer Controller                             │
│  • 创建 ALB (2-3 分钟)                                      │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│                    7. 应用就绪                              │
│  • Pod 运行中                                               │
│  • ALB 健康检查通过                                         │
│  • 可以访问应用                                             │
└─────────────────────────────────────────────────────────────┘
```

## 安全最佳实践

### 1. 凭证管理
- ✅ 使用 IRSA 机制，不硬编码 AWS 凭证
- ✅ 临时凭证自动轮换
- ✅ 不在代码或日志中暴露敏感信息
- ✅ 使用 AWS Secrets Manager 存储敏感配置（可选）

### 2. 网络安全
- ✅ Pod 运行在私有子网，无公网 IP
- ✅ 通过 NAT Gateway 访问互联网
- ✅ ALB 在公有子网，接收外部流量
- ✅ 安全组限制流量（仅允许必要端口）
- ✅ EFS 访问限制在 EKS 节点安全组

### 3. IAM 权限
- ✅ 最小权限原则
- ✅ 每个组件独立的 IAM 角色
- ✅ 明确的资源级权限
- ✅ 定期审计权限使用

### 4. 数据加密
- ✅ EBS 卷加密（静态加密）
- ✅ EFS 文件系统加密（静态加密）
- ✅ S3 存储桶加密（AES256）
- ✅ 传输中加密（HTTPS/TLS）

### 5. 容器安全
- ✅ 使用官方 Python 基础镜像
- ✅ 定期更新依赖包
- ✅ 镜像扫描（ECR 自动扫描）
- ✅ 非 root 用户运行（可选）
- ✅ 只读根文件系统（可选）

### 6. Kubernetes 安全
- ✅ RBAC 权限控制
- ✅ Pod Security Standards
- ✅ Network Policies（可选）
- ✅ 资源限制和配额
- ✅ 健康检查和就绪探针

### 7. 监控和审计
- ✅ CloudWatch 日志集成
- ✅ EKS 控制平面日志
- ✅ CloudTrail API 审计
- ✅ 资源标签用于成本分配

## 使用示例

### 演示存储功能

#### 1. EBS 块存储演示
```bash
# 访问 EBS 页面
curl http://<alb-hostname>/ebs

# 写入数据
curl -X POST http://<alb-hostname>/ebs/write \
  -H "Content-Type: application/json" \
  -d '{"content": "Test data from curl"}'

# 读取数据
curl http://<alb-hostname>/ebs/read
```

#### 2. EFS 共享文件系统演示
```bash
# 访问 EFS 页面
curl http://<alb-hostname>/efs

# 写入文件（多次调用会被不同 Pod 处理）
for i in {1..5}; do
  curl -X POST http://<alb-hostname>/efs/write \
    -H "Content-Type: application/json" \
    -d "{\"content\": \"Test from curl - $i\"}"
  sleep 1
done

# 读取所有文件
curl http://<alb-hostname>/efs/list
```

#### 3. S3 对象存储演示
```bash
# 上传对象
curl -X POST http://<alb-hostname>/s3/upload \
  -H "Content-Type: application/json" \
  -d '{"key": "test.txt", "content": "Hello S3"}'

# 列出对象
curl http://<alb-hostname>/s3/list

# 下载对象
curl http://<alb-hostname>/s3/download/test.txt

# 删除对象
curl -X DELETE http://<alb-hostname>/s3/delete/test.txt
```

### 演示自动扩展

#### 1. 触发 CPU 压力测试
```bash
# 启动 CPU 压力测试
curl -X POST http://<alb-hostname>/stress/cpu \
  -H "Content-Type: application/json" \
  -d '{"duration": 120, "intensity": 80}'

# 观察 HPA 扩展
watch kubectl get hpa -n rj-webdemo

# 查看 Pod 数量变化
watch kubectl get pods -n rj-webdemo
```

#### 2. 触发内存压力测试
```bash
# 启动内存压力测试
curl -X POST http://<alb-hostname>/stress/memory \
  -H "Content-Type: application/json" \
  -d '{"duration": 120, "target_mb": 200}'

# 观察扩展事件
kubectl describe hpa eks-info-app-hpa -n rj-webdemo
```

### 监控和管理

#### 查看应用状态
```bash
# 查看所有 Pod
kubectl get pods -n rj-webdemo -o wide

# 查看 Pod 详细信息
kubectl describe pod <pod-name> -n rj-webdemo

# 查看实时日志
kubectl logs -f deployment/eks-info-app -n rj-webdemo

# 查看多个 Pod 的日志
kubectl logs -f -l app=eks-info-app -n rj-webdemo --max-log-requests=10
```

#### 查看存储状态
```bash
# 查看 PVC
kubectl get pvc -n rj-webdemo

# 查看 PV
kubectl get pv

# 查看 StorageClass
kubectl get storageclass

# 查看 EFS 挂载
kubectl exec -n rj-webdemo -it <pod-name> -- df -h /data/efs

# 查看 EBS 挂载
kubectl exec -n rj-webdemo -it <pod-name> -- df -h /data/ebs
```

#### 查看网络状态
```bash
# 查看 Service
kubectl get svc -n rj-webdemo

# 查看 Ingress
kubectl get ingress -n rj-webdemo

# 查看 ALB 详细信息
kubectl describe ingress eks-info-app-ingress -n rj-webdemo

# 获取 ALB 地址
./scripts/get-alb-hostname.sh
```

#### 手动扩展
```bash
# 扩展到 5 个副本
kubectl scale deployment/eks-info-app --replicas=5 -n rj-webdemo

# 缩减到 2 个副本
kubectl scale deployment/eks-info-app --replicas=2 -n rj-webdemo

# 查看扩展状态
kubectl get deployment eks-info-app -n rj-webdemo
```

### 调试技巧

#### 进入 Pod 调试
```bash
# 进入 Pod Shell
kubectl exec -n rj-webdemo -it <pod-name> -- /bin/bash

# 检查环境变量
kubectl exec -n rj-webdemo -it <pod-name> -- env | grep -E "POD|AWS|S3"

# 测试 S3 访问
kubectl exec -n rj-webdemo -it <pod-name> -- \
  python -c "import boto3; print(boto3.client('s3').list_buckets())"

# 检查存储挂载
kubectl exec -n rj-webdemo -it <pod-name> -- ls -la /data/ebs /data/efs
```

#### 查看事件和日志
```bash
# 查看命名空间事件
kubectl get events -n rj-webdemo --sort-by='.lastTimestamp'

# 查看 Pod 事件
kubectl describe pod <pod-name> -n rj-webdemo | grep -A 10 Events

# 查看 ALB Controller 日志
kubectl logs -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller --tail=100

# 查看 CSI Driver 日志
kubectl logs -n kube-system -l app=ebs-csi-controller --tail=50
kubectl logs -n kube-system -l app=efs-csi-controller --tail=50
```

## 性能基准

### 应用性能
- **启动时间**: 5-10 秒
- **内存使用**: 100-150MB（空闲）
- **CPU 使用**: <5%（空闲）
- **响应时间**: <100ms（本地请求）

### 存储性能
- **EBS gp3**: 3000 IOPS，125 MB/s 吞吐量
- **EFS**: 可扩展，按使用量计费
- **S3**: 高可用，11个9的持久性

### 扩展性能
- **HPA 响应时间**: 30-60 秒
- **Pod 启动时间**: 10-20 秒
- **ALB 健康检查**: 30 秒间隔
- **最大副本数**: 10（可配置）

## 相关文档

- [部署指南](DEPLOYMENT.md) - 完整的部署流程和说明
- [项目总结](PROJECT_SUMMARY.md) - 项目开发历程和总结
- [Bug 修复报告](BUGFIX_REPORT.md) - Bug 修复记录
- [故障排除](TROUBLESHOOTING.md) - 常见问题和解决方案
- [API 文档](eks-info-app/API_DOCUMENTATION.md) - 完整的 API 使用说明
- [Docker 构建指南](eks-info-app/DOCKER_BUILD_GUIDE.md) - Docker 镜像构建详解
- [Kubernetes 资源说明](k8s/README.md) - K8s 资源配置说明
- [需求文档](.kiro/specs/eks-info-webapp/requirements.md) - 功能需求规范
- [设计文档](.kiro/specs/eks-info-webapp/design.md) - 架构设计文档
- [任务列表](.kiro/specs/eks-info-webapp/tasks.md) - 开发任务清单

## 许可证

本项目仅供学习和测试使用。请遵守 AWS 服务条款和相关法律法规。

## 贡献

欢迎提交 Issue 和 Pull Request。

## 联系方式

- **作者**: RJ.Wang
- **邮箱**: wangrenjun@gmail.com
- **项目**: EKS Info WebApp

---

**⚠️ 重要提醒**: 
- 这是一个演示项目，生产环境使用前请进行充分的安全评估和配置调整
- 测试完成后请及时运行 `./scripts/deploy.sh clean` 清理资源，避免产生不必要的费用
- 预估成本约 $2-4/小时，请注意控制使用时间
